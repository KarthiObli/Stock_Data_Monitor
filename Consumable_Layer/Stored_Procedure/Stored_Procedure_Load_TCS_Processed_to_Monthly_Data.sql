CREATE OR REPLACE PROCEDURE "Consumable_Layer"."Stored_Procedure_Load_TCS_Processed_to_Monthly_Data"()
LANGUAGE 'plpgsql'
AS 
$$
BEGIN
TRUNCATE TABLE "Consumable_Layer"."TCS_Monthly_Data";
INSERT INTO "Consumable_Layer"."TCS_Monthly_Data"
(
SELECT "Date",
	CONCAT(
		CASE 
			...
			"Price" FROM "Processed_Layer"."TCS_Processed_Data"  WHERE "Date" in 
				(SELECT "Date" FROM 
					(
					SELECT "Month_Id", MAX("Date") as "Date" from "Processed_Layer"."TCS_Processed_Data"  GROUP BY 1
					)
				)
			ORDER BY "Month_Id"			
	);
