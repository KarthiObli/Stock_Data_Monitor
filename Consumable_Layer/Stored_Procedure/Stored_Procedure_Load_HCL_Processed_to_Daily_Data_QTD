CREATE OR REPLACE PROCEDURE "Consumable_Layer"."Stored_Procedure_Load_HCL_Processed_to_Daily_Data_QTD"()
language plpgsql
AS 
$$
BEGIN
TRUNCATE TABLE "Consumable_Layer"."HCL_Daily_Data_QTD";
INSERT INTO "Consumable_Layer"."HCL_Daily_Data_QTD"
(
WITH a as 
(
	SELECT CASE 
		...
		"Date", 
		"Price" 
		from "Processed_Layer"."HCL_Processed_Data"
		ORDER BY "Date"),
b as
	(
