CREATE OR REPLACE PROCEDURE "Consumable_Layer"."Stored_Procedure_Load_TCS_Processed_to_MonthlyDataAvg"()
LANGUAGE 'plpgsql'
AS 
$$
BEGIN
TRUNCATE TABLE "Consumable_Layer"."TCS_Monthly_Data_Avg";
INSERT INTO "Consumable_Layer"."TCS_Monthly_Data_Avg"
(
SELECT "Date",
	CONCAT(
		CASE 
			...
			"Price"
			FROM (SELECT "Month_Id",MAX("Date") AS "Date", ROUND (AVG("Price"),2) AS "Price" FROM "Processed_Layer"."TCS_Processed_Data" GROUP BY 1 ORDER BY 1)	
	);
END;
$$;
